...
- Production OpenWA API is at `https://server.bhaktienterprises.co.in/openwa/api/`; locally `http://localhost:2785`. PWA `client.ts` uses `VITE_API_BASE` (`.env.production` sets the VPS URL; dev default `/api` via Vite proxy); `vercel.json` rewrites `/api/*` to the VPS as a fallback.
